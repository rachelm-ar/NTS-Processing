### Organising NTS data and calculate mode split for business, commute and other trip purposes (car/rail/bus/walk) ###
...
unclassified_build <- subset(unclassified_build, main_mode != 'unclassified')
...
mode_share_1 <- mode_share %>%
  ...
  select(purpose, main_mode, weighted_trip, soc_cat) %>%
  group_by(purpose, main_mode, soc_cat) %>%
...
mode_share_2 <- mode_share %>%
  filter(purpose == 3 %>%
  filter(ns_sec != -9) %>%
  select(purpose, main_mode, weighted_trip, ns_sec) %>%
  group_by(purpose, main_mode, ns_sec) %>%
  summarise(weighted_group_trips = sum(weighted_trip, na.rm = TRUE)) %>% 
  mutate(share = weighted_group_trips/sum(weighted_group_trips))

# Combine and export
mode_share <- bind_rows(mode_share_1, mode_share_2)
mode_share %>% write_csv("Y:/NTS/mode_time_splits/aggregate_mode_share.csv")
