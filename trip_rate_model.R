### TODO: Figure out why Purpose 8 is not working
###       Apply weights to trip rates

hb_csv_input <- "Y:/NTS/weekly_trip_rates_HA.csv"
...
Process_data <- function(df){
  ...
  transformed_df <- filtered_df %>%
    ...
    mutate(trip_weights = ifelse(weekly_trips == 0, 0, trip_weights/weekly_trips))
  
}

Variable_status <- function(df){
  ...
  #int_model %>% summary()
  #
  #int_model1 <- glm.nb(formula = weekly_trips ~ age_work_status + gender + hh_adults + cars + soc_cat + ns_sec + tfn_area_type,
  #                     data = df,
  #                     subset = train_ind)
  #
  #
  ...
  int_model_vars <- int_model_summ$coefficients$count %>% 
    as.data.frame() %>%
    select("Pr(>|z|)") %>%
...
hb_trip_rates <- function(hb_csv_input, output_dir, ntem_tr_input){
  ...
  for(i in 1:length(purpose_df)){
    ...
    var_status <- Variable_status(purpose_df[[i]])
    ...
    for (j in 1:length(variables)){
      ...
      if(any(var_pvals < 0.01) & var_status[[1]][j] == "configure") {
        ...
        combinations <- compact(combinations)
        
        # If number of combinations is more than 25, sample for 25 - to reduce computation time
        if(length(combinations) > 25){
          
          combinations <- list.sample(combinations, 25)
          
        }
    
        # Update the data frame with aggregated data
        mod_results <- lapply(combinations[[1]], function(x){
          ...
          zinb_mod <- zeroinfl(formula = weekly_trips ~ age_work_status + gender + hh_adults + cars + soc_cat + ns_sec + tfn_area_type | 1,
                               data = updated_df,
                               dist = "negbin",
                               subset = train_ind)
          
          # Extract p-values and obtain number of segments, significant segments and aic
          p_val_summ <- zinb_mod %>% summary()
          
          p_val <- p_val_summ$coefficients$count %>% 
            as.data.frame() %>% 
            select("Pr(>|z|)") %>%
            tibble::rownames_to_column("Variables") %>%
            filter(str_detect(Variables, variables[j]))
    ...
    # Build final model
    final_model[[i]] <- zeroinfl(formula = weekly_trips ~ age_work_status + gender + hh_adults + cars + soc_cat + ns_sec + tfn_area_type | 1,
                         data = final_df[[i]],
                         dist = "negbin",
                         subset = train_ind)
