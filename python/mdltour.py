...
class TourModel:
    ...
    def __init__(self, nts_fldr: str, tmz_leve: str = 'region', over_write: bool = True):
    ...
    def _read_input(self):
        ...
        self.dct_ptaz = self.dfr_ztaz.set_index('tmz_id').to_dict()[f'{self.tmz_leve}_no']
        ...
        self.dfr_ntaz['nts'] = (self.dfr_ntaz[f'{self.taz_leve}_no'] if self.taz_leve == 'region' else
                                self.dfr_ntaz['nts'])
        ...
        if self.tmz_leve.lower() == self.taz_leve.lower():
            ...
            self.dct_ztaz = self.dfr_ztaz.set_index(f'{self.tmz_leve}_no').to_dict()[f'{self.taz_leve}_no']
        ...
        self.nts_list = (self.dfr_tour[self.col_tour].unique(), self.dfr_dist[self.col_dist['o']].unique(),
    ...
    def _aggr_ua1998(self):
        ...
        self.dfr_ntaz['ua_no'], self.dfr_ntaz['ua_de'] = self.dfr_ntaz['ua1998_no'], self.dfr_ntaz['ua1998_de']
        self.dfr_ntaz.loc[self.dfr_ntaz['ua_no'].isin([331, 338, 340]), ['ua_no', 'ua_de']] = [333, 'Highland']
        self.dfr_ntaz.loc[self.dfr_ntaz['ua_no'].isin([59]), ['ua_no', 'ua_de']] = [55, 'Bournemouth']

        if 'ua_no' in self.dfr_ztaz.columns:
            self.dfr_ztaz['ua_no'], self.dfr_ztaz['ua_de'] = self.dfr_ztaz['ua1998_no'], self.dfr_ztaz['ua1998_de']
            self.dfr_ztaz.loc[self.dfr_ztaz['ua_no'].isin([331, 338, 340]), ['ua_no', 'ua_de']] = [333, 'Highland']
            self.dfr_ztaz.loc[self.dfr_ztaz['ua_no'].isin([59]), ['ua_no', 'ua_de']] = [55, 'Bournemouth']

    # CALCULATE PRODUCTION
    def _calc_prod(self, for_test: bool = False):
        fun.log_stderr(f' .. HB starting trip-ends ...')
        # convert NTEM data to standard format
        self.dfr_prod = self._stack_dfr(self.dfr_prod, [self.col_prod, 'mode', 'purpose', 'direction'])
    ...
    def _calc_rezone(self, agg_rail: bool = False):
        ...
        # mode-time split
        if agg_rail:
            self.dfr_dist.loc[self.dfr_dist['mode'] == 7, 'mode'] = 6  # as input trip-ends combine 6 & 7
        col_indx = [self.col_dist['o'], self.col_dist['d']]
        self.dfr_dist = self.dfr_dist.set_index(col_indx).rename(index=self.dct_ntaz).reset_index()
        col_grby = [self.col_dist['o'], self.col_dist['d'], 'mode', 'purpose', 'direction', 'period', 'trav_dist']
        self.dfr_dist = self.dfr_dist.groupby(by=col_grby, observed=True).sum()
        self.dfr_dist = self.dfr_dist.reset_index()
        # tour
        self.dfr_tour = self.dfr_tour.set_index(self.col_tour).rename(index=self.dct_ntaz).reset_index()
    ...
    def _calc_mts(self, col_used: str = 'freq', for_test: bool = False):
        ...
        col_grby = [self.col_dist['o'], self.col_dist['d'], 'mode', 'purpose', 'direction', 'period']
    ...
    def _calc_dest(self, col_used: str = 'freq', for_test: bool = False):
        ...
        dfr_temp = dfr_dist.groupby(by=[self.col_dist['o'], 'purpose', 'direction'])[col_used].transform('sum')
    ...
    def _calc_tour(self, val_incl: float = 0, for_test: bool = False):
        ...
        fun.log_stderr(f'    {len(self.uni_tour)} tour_ids (with {val_incl}+ trips) selected, '
        ...
        for tor in self.uni_tour:
            ...
            est_tour.append(pool.apply_async(self._tour_breakdown, [tor, pp_first, 'freq']))
        pool.close()
        pool.join()
    ...
    def _tour_breakdown(self, tor: str, pp_first: int, col_used: str) -> pd.DataFrame:
        ...
        if pp_first in self.ppx_list:
            for taz in self.taz_list:
                dfr_value = self.dfr_tour.loc[(self.dfr_tour['tour_id'] == tor) &
                                              (self.dfr_tour[self.col_tour] == taz)][col_used].values
                dfr_temp.append([taz, pp_first, dfr_value[0] if len(dfr_value) > 0 else 0])
        dfr_temp = pd.DataFrame(data=dfr_temp, columns=['taz', 'purpose', tor]).set_index(['taz', 'purpose'])
        return dfr_temp
    ...
    def _calc_trip_by_activity(self):
        ...
        for tor in self.uni_tour:
            ...
            pp_first = act_list[1]
            if pp_first in self.ppx_list:
                est_trip.append(pool.apply_async(self._activity_breakdown, [tor, act_list]))
        ...
        for leg in range(1, len(act_list)):
            ...
            if pp_start == 0:  # home-based: from-home trip,use ppx_next
                for tmz_o in self.tmz_list:
                    ...
                    tmz_prod = self._extract_prod(tmz_o, ppx_next) * self._extract_tour(taz_o, ppx_next, tor)
                    if tmz_prod > 0:
                        for tmz_d in self.tmz_list:
                            ...
                            for md in self.mdx_list:
                                for ts in self.tsx_list:
                                    taz_dist = self._extract_dest(taz_o, ppx_next, dir_type, taz_d)
                                    taz_fact = self._extract_mts(taz_o, taz_d, ppx_next, dir_type, md, ts)
                                    tmz_trip = tmz_prod * taz_dist * taz_fact
                                    tmz_attr[leg][tmz_d] += tmz_trip
                                    dfr_temp.append([tmz_o, tmz_d, tor, md, act, dir_type, ts, tmz_trip])
                        ...
                        for tmz_d in self.tmz_list:
                            ...
                            for md in self.mdx_list:
                                for ts in self.tsx_list:
                                    taz_dist = self._extract_dest(taz_o, ppx_next, dir_type, taz_d)
                                    taz_fact = self._extract_mts(taz_o, taz_d, ppx_next, dir_type, md, ts)
                                    tmz_trip = tmz_attr[leg - 1][tmz_o] * taz_dist * taz_fact
                                    tmz_attr[leg][tmz_d] += tmz_trip
                                    dfr_temp.append([tmz_o, tmz_d, tor, md, f'x{act[1:]}', dir_type, ts, tmz_trip])
                ...
                for tmz_o in self.tmz_list:
                    ...
                    if tmz_attr[leg - 1][tmz_o] > 0:
                        for tmz_d in self.tmz_list:
                            ...
                            for md in self.mdx_list:
                                for ts in self.tsx_list:
                                    taz_dist = self._extract_dest(taz_o, pp_start, dir_type, taz_d)
                                    taz_fact = self._extract_mts(taz_o, taz_d, pp_start, dir_type, md, ts)
                                    tmz_trip = tmz_attr[leg - 1][tmz_o] * taz_dist * taz_fact
                                    dfr_temp.append([tmz_o, tmz_d, tor, md, act, dir_type, ts, tmz_trip])
            # create dataframe
    ...
    def _adjust_tripend(self):
        ...
        pool = mp.Pool(2)
        par_pool = ['prod', 'hb_fr', self._adjust_aggs(self.dfr_prod, 'both'), 'both',
                    fr'{cur_path}\productions_hb_fr.csv']
        out_data['o'].append(pool.apply_async(self._adjust_engine, par_pool))
        par_pool = ['attr', 'hb_fr', self._adjust_aggs(self.dfr_attr, 'both'), 'both',
                    fr'{cur_path}\attractions_hb_fr.csv']
        ...
        pool = mp.Pool(2)
        par_pool = ['orig', 'nhb', self._adjust_aggs(fr'{cur_path}\attractions_hb_fr.csv', 'both'), 'both',
                    fr'{cur_path}\productions_nhb.csv']
        out_data['o'].append(pool.apply_async(self._adjust_engine, par_pool))
        par_pool = ['dest', 'nhb', self._adjust_aggs(fr'{cur_path}\attractions_hb_fr.csv', 'both'), 'both',
                    fr'{cur_path}\attractions_nhb.csv']
        out_data['d'].append(pool.apply_async(self._adjust_engine, par_pool))
        pool.close()
        pool.join()
        # hb_to (OD - mode agnostic by time period to mode specific & time period)
        pool = mp.Pool(2)
        par_pool = ['orig', 'hb_to', self._adjust_aggs(fr'{cur_path}\attractions_nhb.csv', 'both'), 'both',
                    fr'{cur_path}\productions_hb_to.csv']
        out_data['o'].append(pool.apply_async(self._adjust_engine, par_pool))
        par_pool = ['dest', 'hb_to', self._adjust_aggs(fr'{cur_path}\productions_hb_fr.csv', 'both'), 'both',
                    fr'{cur_path}\attractions_hb_to.csv']
        out_data['d'].append(pool.apply_async(self._adjust_engine, par_pool))
        pool.close()
        pool.join()
        # summary
    ...
    def _stack_dfr(dfr: pd.DataFrame, col_grby: Union[List, str]) -> pd.DataFrame:
        lev = len(col_grby)
        dfr = dfr.set_index(col_grby).stack().reset_index(name='val').rename(columns={f'level_{lev}': 'period'})
        dfr['period'] = dfr['period'].astype(int)
        return dfr.groupby(col_grby + ['period'])[['val']].sum().reset_index()
        
    # internal function 1
    def _adjust_aggs(self, csv_file: Union[pd.DataFrame, str], agg_type: str = 'none') -> pd.DataFrame:
        ...
        if agg_type.lower() == 'mode':
            dfr_data = dfr_data.groupby(by=[self.col_prod, 'p', 'tp'])[['val']].sum().reset_index()
        elif agg_type.lower() == 'time':
            dfr_data = dfr_data.groupby(by=[self.col_prod, 'p', 'm'])[['val']].sum().reset_index()
        elif agg_type.lower() == 'both':
            dfr_data = dfr_data.groupby(by=[self.col_prod, 'p'])[['val']].sum().reset_index()
        return dfr_data
    ...
    def _adjust_fill(dfr_data: pd.DataFrame, col_dict: Dict) -> pd.DataFrame:
        ...
        col_dict = {col: list(col_dict[col]) if type(col_dict[col]) is not list else col_dict[col]
                    for col in col_dict}
        num_2add = np.prod([len(col_dict[key]) for key in col_dict])
        col_dict = {'col': [[key for key in col_dict] for _ in range(num_2add)],
    ...
    # internal function 3
    def _adjust_engine(self, ted_type: str, dir_type: str, dfr_data: pd.DataFrame, col_2add: str,
        ...
        est_trip = self.est_trip.loc[self.est_trip['direction'] == dir_type].reset_index(drop=True)
        est_trip['purpose'] = (est_trip['purpose'].apply(lambda x: int(x[-1])) if dir_type in ['hb_fr', 'nhb'] else
                               est_trip['purpose'].apply(lambda x: int(x[0])))
        est_trip['purpose'] = est_trip['purpose'] + (10 if dir_type == 'nhb' else 0)
        tmz_type = 'tmz_o' if ted_type in ['prod', 'orig'] else 'tmz_d'
        est_trip = est_trip.groupby(by=[tmz_type, 'mode', 'purpose', 'period'])[['trips']].sum().reset_index()
        ...
        # rezoning
        dfr_tend['p'] = dfr_tend['p'] + (10 if dir_type == 'nhb' and dfr_tend['p'].max() < 10 else
                                         -10 if dir_type != 'nhb' and dfr_tend['p'].min() > 10 else 0)
        dfr_tend[self.tmz_leve] = dfr_tend[self.col_prod]
        dfr_tend = dfr_tend.set_index(self.tmz_leve).rename(index=self.dct_ptaz).reset_index()
        # merge data
        dfr_tend = pd.merge(dfr_tend, est_trip, how='left', left_on=[self.tmz_leve, 'm', 'p', 'tp'],
                            right_on=[tmz_type, 'mode', 'purpose', 'period']).fillna(0)
        dfr_tend.drop(columns=[tmz_type, 'mode', 'purpose', 'period'], inplace=True)
        # apply mode-time split from est_trip
        if col_2add.lower() == 'mode':
            dfr_temp = dfr_tend.groupby(by=[self.col_prod, 'p', 'tp'])['trips'].transform('sum')
            dfr_tend['val'] = dfr_tend['val'].div(dfr_temp).mul(dfr_tend['trips']).fillna(0)
        elif col_2add.lower == 'time':
            dfr_temp = dfr_tend.groupby(by=[self.col_prod, 'p', 'm'])['trips'].transform('sum')
            dfr_tend['val'] = dfr_tend['val'].div(dfr_temp).mul(dfr_tend['trips']).fillna(0)
        elif col_2add.lower() == 'both':
            dfr_temp = dfr_tend.groupby(by=[self.col_prod, 'p'])['trips'].transform('sum')
            dfr_tend['val'] = dfr_tend['val'].div(dfr_temp).mul(dfr_tend['trips']).fillna(0)
        # control to est_trip totals
        dfr_temp = dfr_tend.groupby(by=[self.tmz_leve, 'p', 'm', 'tp'])['val'].transform('sum')
        dfr_tend['val'] = dfr_tend['val'].div(dfr_temp).mul(dfr_tend['trips']).fillna(0)
    ...
    def _reports(self):
        ...
        col_mode, col_purp, col_zone = self._col_mode(True), self._col_purp(), self._col_zone(self.tmz_leve)
        est_trip = self.est_trip.set_index(['purpose']).rename(index=self._act_to_purp()).reset_index()

        # p/a trip-ends
        self.dfr_attr = self._stack_dfr(self.dfr_attr, [self.col_prod, 'mode', 'purpose', 'direction'])
        nts_trip = {'o': self.dfr_prod, 'd': self.dfr_attr}
        for od, odx in zip(['o', 'd'], ['prod', 'attr']):
            ted_data = est_trip.groupby([f'tmz_{od}'] + col_grby)['trips'].sum().reset_index()
            nts_data = nts_trip[od].rename(columns={self.col_prod: f'tmz_{od}'})
            ted_data = pd.merge(ted_data, nts_data, how='outer', on=[f'tmz_{od}'] + col_grby).fillna(0)
    ...
    def _extract_mts(self, taz_o: int, taz_d: int, pp: int, direction: str, md: int, time_start: int) -> float:
        """
            mode-time probability: mts[i, j, p, d, m, t]
        """
        return self.dct_mode.get((taz_o, taz_d, pp, direction, md, time_start), 0)

    def _extract_dest(self, taz_o: int, pp: int, direction: str, taz_d: int, taz_prop: float = 1) -> float:
        """
    ...
    def _read_csv(csv_file: str) -> pd.DataFrame:
        ...
        dfr_data.rename(columns={key: key.lower() for key in dfr_data.columns}, inplace=True)
        return dfr_data
    ...
    @staticmethod
    def _col_purp():
        return {1: 'Commuting', 2: 'Emp.business', 3: 'Education', 4: 'Shopping', 5: 'Per.business',
                6: 'Social', 7: 'Visit friends', 8: 'Holiday', 9: 'NA'}

    @staticmethod
    def _col_purp_old():
        col_name = {}
        ppx_name = {1: 'w', 2: 'eb', 3: 'edu', 4: 'shop', 5: 'pb', 6: 'social', 7: 'vf', 8: 'hol', 9: 'esc'}
    ...
    def _dup_to_tuple(val_list: List) -> List:
        ...
        return [(key, sum(1 for _ in group)) for key, group in itertools.groupby(val_list)]
...
class MSOAtoNoRMS:
    def __init__(self):
        ...
        self.csv_prod = r'D:\NTS\NoTEM\tripend\full_week_hb_production_9-10_p_m_msoa.csv'
        self.csv_attr = r'D:\NTS\NoTEM\tripend\full_week_hb_attraction_9-10_p_m_msoa.csv'
    ...
    def _msoa_to_norms(self):
        ...
        for key, lok, ted in zip(['production', 'attraction'], lok_list, ted_list):
            ...
            sec_2tmz = tmz_tran.groupby('msoa_zone_id')['msoa_to_norms'].transform('sum')
            tmz_tran['msoa_to_norms'] = tmz_tran['msoa_to_norms'].div(sec_2tmz).fillna(0)
            # read trip-end file
            ted_data = self._read_csv(ted)
            sum_pres = ted_data.groupby(['m', 'p'])[['val']].sum().rename(columns={'val': 'pres'})
            ted_data = pd.merge(tmz_tran, ted_data, how='left', on='msoa_zone_id', suffixes=('', ''))
            ted_data['val'] = ted_data['val'].mul(ted_data['msoa_to_norms']).fillna(0)
            ted_data = ted_data.groupby(['norms_zone_id', 'p', 'm'])['val'].sum().reset_index()
            # check
            sum_post = ted_data.groupby(['m', 'p'])[['val']].sum().rename(columns={'val': 'post'})
            sum_post = pd.concat([sum_pres, sum_post], axis=1)
            sum_post['diff'] = sum_post['post'] - sum_post['pres']
            ted_data.to_csv(fr'{self.nts_fldr}\tripend\full_week_hb_{key}_9-10_p_m_norms.csv', index=False)
