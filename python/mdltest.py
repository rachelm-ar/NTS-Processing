...
class Test:
    ...
    def __init__(self, nts_fldr: str, cb_version: Union[str, int], over_write: bool = True):
        ...
        if over_write:
            ...
            self._trip_length(nts_data, self.tfn_mode, 'gor', 'ruc_tld', True)
    ...
    def _escort(self, dfr_data: pd.DataFrame) -> pd.DataFrame:
        # process data
        col_grby = ['householdid', 'individualid', 'purpose', 'escort', 'period', 'direction',
                    'age_edu', 'trav_dist']
        dfr = dfr_data.groupby(col_grby)['w2'].mean().reset_index()
        dfr = dfr.loc[dfr['purpose'] == 3].reset_index(drop=True).drop(columns='purpose')
        col_indx = {key: f'{idx}.{key}' for idx, key in enumerate(['hb_fr', 'nhb', 'hb_to'], 1)}
        dfr = dfr.set_index('direction').rename(index=col_indx).reset_index()
        col_grby = ['householdid', 'individualid', 'period', 'direction']
        dfr = dfr.set_index(col_grby).sort_index().reset_index()

        # create lookup
        dct = dfr.loc[dfr['escort'] == 'main'].reset_index(drop=True)
        col_grby = ['householdid', 'period', 'direction', 'trav_dist', 'age_edu']
        dct = dct.groupby(col_grby)['escort'].count().reset_index().rename(columns={'escort': 'prop'})
        dct['prop'] = dct['prop'].div(dct.groupby(col_grby[:-1])['prop'].transform('sum')).fillna(0)
        dct = pd.pivot_table(dct, index=col_grby[:-1], columns=col_grby[-1], values='prop').fillna(0)
        edu_list = list(dct.columns)

        # apply split
        dfr.drop(columns=edu_list, inplace=True, errors='ignore')
        dfr.loc[dfr['escort'] == 'main', edu_list] = 0
        for col in edu_list:
            dfr.loc[(dfr['escort'] == 'main') & (dfr['age_edu'] == col), col] = 1
        for row in range(3):
            out = dfr[col_grby[1:-1]].shift(periods=-row, fill_value=0)
            out = pd.merge(pd.concat([dfr['householdid'], out], axis=1), dct, how='left', left_on=col_grby[:-1],
                           right_on=col_grby[:-1])[edu_list]
            dfr.loc[(dfr[edu_list[0]].isna()) & (dfr['escort'] != 'main'), edu_list] = out

        # infill missing data based on household
        for col_grby in (['householdid', 'direction'], ['householdid']):
            out = dfr.loc[dfr['escort'] == 'main'].reset_index(drop=True)
            out = out.groupby(col_grby)[edu_list].sum()
            out = (out.stack(future_stack=True).reset_index(name='prop')
                   .rename(columns={f'level_{len(col_grby)}': 'age_edu'}))
            out['prop'] = out['prop'].div(out['age_edu'])
            out['prop'] = out['prop'].div(out.groupby(col_grby)['prop'].transform('sum')).fillna(0)
            out = pd.pivot_table(out, index=col_grby, columns='age_edu', values='prop').fillna(0)
            out = pd.merge(dfr[col_grby], out, how='left', left_on=col_grby, right_on=col_grby)[edu_list]
            dfr.loc[(dfr[edu_list[0]].isna()) & (dfr['escort'] != 'main'), edu_list] = out

        # infill missing data based on distance (simplified for now)
        for col, val in zip(edu_list, [(0, 10), (10, 20), (20, 999)]):
            dfr.loc[(dfr[col].isna()) & (dfr['trav_dist'] >= val[0]) & (dfr['trav_dist'] < val[1]), col] = 1

        # stack dataframe
        col_grby = ['householdid', 'individualid', 'period', 'trav_dist', 'direction', 'escort']
        dfr = (dfr[col_grby + edu_list].groupby(col_grby)[edu_list].sum()
               .stack(future_stack=True).reset_index(name='prop')
               .rename(columns={f'level_{len(col_grby)}': 'age_edu'})).fillna(0)
        dfr = dfr.loc[dfr['prop'] > 0].reset_index(drop=True)
        dfr = dfr.set_index('direction').rename(index=self.luk.val_to_key(col_indx)).reset_index()
        return dfr.drop(columns='householdid')

    def _trip_length(self, dfr_data: pd.DataFrame, mode: Union[List, None] = None, geo_incl: Union[str, None] = None,
        ...
        dfr = dfr_data[col_used + seg_incl + ['trips']].copy()
    ...
    def _trip_rates_production(self, dfr_data: pd.DataFrame, mode: Union[List, None] = None,
        ...
        dfr = dfr_data[col_used + seg_incl + ['individualid', 'w2', 'trips']].copy()
    ...
    def _trip_rates_attraction(self, dfr_data: pd.DataFrame, mode: Union[List, None] = None,
                               geo_incl: Union[str, None] = None, seg_incl: Union[List, str, None] = None):
        # trip attraction rates Alpha1 (by purpose, land-use type, soc, and area_type)
        fun.log_stderr('\nNTS trip rates (attractions)')
        fun.log_stderr(f' .. escort trips by edu age')
        esc_trip = self._escort(dfr_data)

        fun.log_stderr(f' .. process data')
        lev_2col, col_type = self.luk.lev_to_name(geo_incl), self.luk.nts_dtype
        seg_incl = fun.str_to_list(seg_incl) if seg_incl is not None else []
        lev_prod, lev_dest = lev_2col['h'], lev_2col['d']
        col_xtra = ['age_edu', 'escort', 'indwkgor_b02id', 'indwkcounty_b01id', 'indwkurban_b01id',
                    'indwkua1998_b01id', 'indwkua2009_b01id', 'indwkareatype1_b01id', 'indwkareatype2_b01id']
        col_used = col_xtra + [lev_dest, 'mode', 'purpose', 'direction', 'individualid', 'w2', 'trips']
        dfr = dfr_data[seg_incl + col_used + ['trav_dist', 'period']].copy()
        mode = self.tfn_mode if mode is None else mode
        dfr = dfr.loc[dfr['direction'].isin(['hb_fr', 'nhb'])]
        # trip-rates (based on number of employment)
        col_grby = seg_incl + col_xtra + [lev_dest, 'direction']
        ...
        edu = emp.loc[emp['purpose'] == 3].drop(columns='purpose')
        emp = emp.loc[emp['purpose'] == 1].drop(columns='purpose')
        # emp & edu data
        col_grby = seg_incl + [lev_dest, 'age_edu', 'escort', 'direction']
        emp = emp.groupby(col_grby)[['w2']].sum()
        edu = edu.groupby(col_grby)[['w2']].sum()
        # trip data
        dfr = fun.dfr_filter_mode(dfr, mode)
        ppx_list = dfr['purpose'].unique()
        col_join = ['individualid', 'period', 'direction', 'escort', 'trav_dist']
        dfr = pd.merge(dfr, esc_trip, how='left', left_on=col_join, right_on=col_join, suffixes=('', '_esc'))
        dfr.loc[~dfr['age_edu_esc'].isna(), 'age_edu'] = dfr['age_edu_esc']
        dfr['trips'] = dfr['trips'].mul(dfr['prop'].fillna(1))
        ...
        # test
        # dfr.loc[(dfr['individualid'] == 2002000049) & (dfr['purpose'] == 3)][col_join + ['purpose', 'age_edu']]
        # out = (pd.merge(dfr, esc_trip, how='left', left_on=col_join, right_on=col_join, suffixes=('', '_esc'))
        #        [col_join + ['purpose', 'age_edu', 'age_edu_esc', 'prop']])
        # out = out.loc[(out['individualid'] == 2002000049) & (out['purpose'] == 3)]

        # write output
        fun.log_stderr(f' .. write output')
        out_fldr = fr'{self.cfg.dir_output}\{self.cfg.fld_attr}'
        fun.dfr_to_csv(dfr.sort_index(), out_fldr, 'trip_rates_attractions', True)
        fun.dfr_to_csv(edu.sort_index(), out_fldr, 'education_attractions', True)

    def _tour_proportion(self, dfr_data: pd.DataFrame, mode: Union[List, None] = None,
                         geo_incl: Union[str, None] = None, seg_incl: Union[List, str, None] = None):
        # geo_incl: geo_area to be included: either gor, county, tfn_at
        ...
        col_used = [lev_prod] + col_used if geo_incl is not None else col_used
        dfr = dfr_data[col_used + seg_incl + ['individualid', 'direction', 'tour_group', 'trips']].copy()
    ...
    def _veh_occupancy(self, dfr_data: pd.DataFrame, mode: Union[List, None] = None, geo_incl: Union[str, None] = None,
    ...
    def _activity(self, dfr_data: pd.DataFrame, mode: Union[List, None] = None, geo_incl: Union[str, None] = None,
        ...
        dfr = dfr_data[col_used + seg_incl + ['w2', 'trips']].copy()
    ...
    def _mts_hbase(self, dfr_data: pd.DataFrame, mode: Union[List, None] = None, geo_incl: Union[str, None] = None,
        ...
        mode = self.tfn_mode if mode is None else mode
        dfr = dfr_data.loc[(dfr_data['direction'] == 'hb_fr') & (dfr_data['w1'] == 1)].copy()
    ...
    def _trip_rates_nhbase(self, dfr_data: pd.DataFrame, mode: Union[List, None] = None,
                           geo_incl: Union[str, None] = None, seg_incl: Union[List, str, None] = None):
        # geo_incl: geo_area to be included: either gor, county, tfn_at
        ...
        col_used = seg_incl + [col for col in self.tfn_ttype if col not in seg_incl]
        dfr = dfr_data[col_used + ['direction', 'period', 'tour_group', 'individualid', 'trips']].copy()
        mode = self.tfn_mode if mode is None else mode
    ...
    def _mts_nhbase(self, dfr_data: pd.DataFrame, mode: Union[List, None] = None, geo_incl: Union[str, None] = None,
                    seg_incl: Union[List, str, None] = None):
        # geo_incl: geo_area to be included: either gor, county, tfn_at
        ...
        col_used = seg_incl + [col for col in self.tfn_ttype if col not in seg_incl]
        dfr = dfr_data[col_used + ['direction', 'period', 'individualid', 'trips']].copy()
        mode = self.tfn_mode if mode is None else mode
    ...
    def _mts_attraction(self, dfr_data: pd.DataFrame, mode: Union[List, None] = None, geo_incl: Union[str, None] = None,
                        seg_incl: Union[List, str, None] = None):
        ...
        dfr = dfr_data[seg_incl + ['mode', 'purpose', 'direction', 'period', 'trips']].copy()
        mode = self.tfn_mode if mode is None else mode
        dfr = dfr.loc[dfr['direction'].isin(['hb_fr', 'nhb'])]
