...
class TripRate:
    ...
    def _install_r(upgrade: bool = False):
        ...
        for pkg in ("MASS", "pscl"):
            if not rpkg.isinstalled(pkg) or upgrade:
                ...
                fun.log_stderr(
                    f" .. {pkg} - current version installed: {_version(pkg)}"
                )
            rpkg.importr(pkg)
        fun.log_stderr(
            f" .. !!! be mindful of result differences when upgrading to a newer version !!!"
        )
        pd2r.activate()
    ...
    def _hb_trip_rates(
        self,
        dfr: pd.DataFrame,
        reg_grby: Union[List, str, None] = None,
        out_2csv: bool = False,
    ):
        fun.log_stderr("\nProcess NTS data")
        ...
        dfr = (
            dfr.groupby(col_grby, observed=True)[["w1", "jjxsc", "w5xhh", "trips"]]
            .sum()
            .reset_index()
        )
        ...
        out = pd.pivot_table(
            out, values="trips", index=col_grby, columns=reg_grby
        ).fillna(0)
        reg_name = [
            col[0] for col in [col if col != "period" else "time" for col in reg_grby]
        ]
        if len(reg_grby) > 1:
            out.columns = out.columns.map(
                lambda x: ".".join([f"{cx}{cv}" for cv, cx in zip(x, reg_name)])
            )
        else:
        ...
        out = pd.merge(
            ...
            out,
            how="right",
            on=["tt", "tfn_at"] + self.tfn_ttype,
            suffixes=("", ""),
        )
        out = pd.merge(
            out, self.dfr_ttype, how="outer", on=["tt"] + self.cfg.tfn_ttype
        ).fillna(0)
        ...
        out[reg_grby] = (
            out["segment"].str.split(".", expand=True)
            if len(reg_grby) > 1
            else out[["segment"]]
        )
        for col in reg_grby:
        ...
        out.rename(
            columns={"jjxsc": "weekly_trips", "trips": "trips_nts"}, inplace=True
        )
        fun.dfr_to_csv(
            out, out_fldr, f"hb_trip_rates_build", True
        ) if out_2csv else None
        self.obs_trip = out.reset_index()
        # response weight
        fun.log_stderr(f" .. response weights")
        dfr["w2"] = dfr["w2"].mul(dfr["jjxsc"]).fillna(0)  # weighted individual
        dfr = dfr.groupby(["surveyyear", "purpose"], observed=True)[
            ["w1", "w2", "jjxsc", "trips"]
        ].sum()
        dfr.rename(columns={"trips": "trips_nts"}, inplace=True)
        fun.dfr_to_csv(
            dfr, out_fldr, f"hb_trip_rates_response_weights", True
        ) if out_2csv else None
    ...
    def _regx_form(self, reg_grby: Union[List, str, None] = None):
        def _str_remove(str_main: str, str_removed: str) -> str:
            return str_main.replace(f" + {str_removed}", "").replace(
                f"{str_removed} + ", ""
            )
        ...
        tfn_ttype = _str_remove(
            _str_remove(tfn_ttype, "purpose"), "aws"
        )  # purpose & aws are part of regression
        for pp in self.ppx_list:
            ...
            for aws in self.aws_list:
                ...
                inf_zero = (
                    " | 1" if mdl_form != "nb" else ""
                )  # for zero-inflated models
                mdl_spec = (
                    _str_remove(tfn_ttype, "gender") if aws in [1] else tfn_ttype
                )  # a child has no gender
                mdl_spec = (
                    _str_remove(mdl_spec, "soc") if aws in [1, 4, 5, 6] else mdl_spec
                )
                self.mdl_form[pp][aws]["spec"] = f"{mdl_spec}{inf_zero}"

    def _regx_model_rs(self, reg_grby: Union[List, str, None] = None):
        """
        Calls code written in R to form regression models for trip-rates.

        This is based on a methodology developed by Atkins, Aecom and Imperial College.
        """
        fun.log_stderr(f"\nRegression model (R)")
        ...
        for pp in self.ppx_list:
            ...
            for aws in self.aws_list:
                mdl_form, mdl_spec = (
                    self.mdl_form[pp][aws]["form"],
                    self.mdl_form[pp][aws]["spec"],
                )
                ...
                old = old.groupby(
                    tfn_ttype + ["surveyyear"] + reg_grby, observed=False
                )[["trip_rates"]].mean()
                old = pd.merge(
                    old.reset_index(),
                    obs_resw,
                    how="left",
                    on=["surveyyear", "purpose"],
                )
                yrs_fact = (
                    old["w1"]
                    .div(old.groupby(col_grby, observed=True)["w1"].transform("sum"))
                    .fillna(1)
                )
                ...
                self.unw_trip.append(
                    dfr.groupby(["aws"] + reg_grby)[["trip_rates"]].mean()
                )
        ...
        self.reg_stat = self.reg_stat.groupby(
            ["purpose", "aws"] + reg_grby, observed=False
        )[col_item].sum()
        fun.dfr_to_csv(self.reg_stat, out_fldr, "Regression_summary")

    def _regx_model_py(self, reg_grby: Union[List, str, None] = None):
        """
        Regression models written in python.

        These models don't work as well as the R versions, but may be a more
        readable version of what is being attempted.
        """
        fun.log_stderr(f"\nRegression model (python)")
        obs_trip, obs_resw = self.obs_trip, self.obs_resw
        reg_grby = [] if reg_grby is None else fun.str_to_list(reg_grby)
        self.unw_trip, self.reg_trip, tfn_ttype = (
            [],
            [],
            self.tfn_ttype + ["tfn_at"] + reg_grby,
        )
        for pp in self.ppx_list:
            for aws in self.aws_list:
                mdl_form, mdl_spec = (
                ...
                fun.log_stderr(f" .. purpose {pp} / aws {aws} ({mdl_form.upper()})")
                dfr = obs_trip.loc[
                    (obs_trip["purpose"] == pp) & (obs_trip["aws"] == aws)
                ]
                dfr = dfr.reset_index(drop=True)
                dfr = self._regx_engine_py(mdl_form, mdl_spec, dfr, tfn_ttype)
                # un-weighted trip-rates
                self.unw_trip.append(
                    dfr.groupby(["purpose", "aws"], observed=True)[["trips"]].mean()
                )
                ...
                dfr_temp = dfr.groupby(tfn_ttype + ["purpose"], observed=True)[
                    "count"
                ].transform("sum")
                dfr["count"] = dfr["count"].div(dfr_temp)
                dfr["trips"] = dfr["trips"].mul(dfr["r_weights"]).mul(dfr["count"])
                dfr = dfr.groupby(tfn_ttype + ["purpose"], observed=True)[
                    ["trips"]
                ].sum()
    ...
    def _regx_output(self, reg_type: str = None):
        ...
        self.unw_trip = self.unw_trip.pivot(
            columns="aws", index="purpose", values="trip_rates"
        )
        ...
        fun.dfr_to_csv(
            self.reg_trip.rename(columns={0: "trip_rates"}),
            out_fldr,
            "trip_rates_hb_weighted",
        )

    def _analysis(self, reg_type: str = None):
        """Output statistics and graphs on regression models."""
        fun.log_stderr(f"\nAnalysis")
        tfn_atyp = self.cfg.tfn_atype
        tfn_ttype = self.tfn_ttype + ["tfn_at", "purpose"]
        out_fldr = self.fld_prod / self.cfg.fld_hbase / self.cfg.fld_rates
        nts_trip = fun.csv_to_dfr(
            out_fldr / "trip_rates_hb_long.csv", tfn_ttype + ["w2", "trip_rates"]
        )
        cte_trip = fun.csv_to_dfr(
            self.cfg.dir_import / "trip_rates_hb_ctripend.csv", tfn_ttype + ["trips"]
        )
        ...
        reg_trip.rename(
            columns={"trip_rates": "trips", "trip_rates_old": "trips_old"}, inplace=True
        )
        all_trip = pd.merge(
            nts_trip, reg_trip, how="outer", on=tfn_ttype, suffixes=("_nts", "_reg")
        ).fillna(0)
    ...
    def _compare_python_vs_r(self):
        """Compare python and r regression results."""
        fun.log_stderr(f"\nAnalysis")
        tfn_ttype = self.tfn_ttype + ["tfn_at", "purpose"]
        out_fldr = self.fld_prod / self.cfg.fld_hbase
        pyx_trip = fun.csv_to_dfr(
            out_fldr / "python" / "trip_rates_hb_weighted.csv", tfn_ttype + ["trips"]
        )
        rst_trip = fun.csv_to_dfr(
            out_fldr / "rstudio" / "trip_rates_hb_weighted.csv", tfn_ttype + ["trips"]
        )
        # merge database
        all_trip = pd.merge(
            pyx_trip, rst_trip, how="left", on=tfn_ttype, suffixes=("_py", "_rs")
        )
        # output scatter plots
        for pp in all_trip["purpose"].unique():
            ...
            fun.plt_scatter(
                f"p{pp}_aws0", pyx, rst, "PY", "RS", out_fldr / self.cfg.fld_graph
            )
            for aws in all_trip["aws"].unique():
                mdl_form = self.mdl_form[pp][aws]["form"]
                dfr = all_trip.loc[
                    (all_trip["purpose"] == pp) & (all_trip["aws"] == aws)
                ].reset_index(drop=True)
                pyx, rst = dfr["trips_py"].values, dfr["trips_rs"].values
                fun.plt_scatter(
                    f"p{pp}_aws{aws}_{mdl_form}",
                    pyx,
                    rst,
                    "PY",
                    "RS",
    ...
    def _regx_engine_rs_nw(
        ...
        col_year = (
            ["purpose", "surveyyear"] if "surveyyear" in col_grby else ["purpose"]
        )
        dfr["w1"] = 1
        ...
        dfr = (
            dfr.groupby(tfn_ttype + col_year, observed=False)
            .agg(agg_func)
            .reset_index()
        )
        w5x_fill = self._nan_fill(dfr, col_grby, "w5_weights", "w2_weights")
        dfr["w5_weights"] = (
            dfr["w5_weights"].div(dfr["w2_weights"]).fillna(w5x_fill)
        )  # response weights
        ...
        dfr["trip_rates"] = dfr["trip_rates"].mul(
            dfr["w5_weights"] if method != "old" else 1
        )
        dfr["w2_weights"] = np.where(dfr["w2"] > 0, dfr["w2"], 0.1 * dfr["w2_weights"])
        dfr[col_grby] = dfr[col_grby].astype(int)
        dfr["surveyyear"] = (
            dfr["surveyyear"].astype(int) if "surveyyear" in col_grby else 0
        )
    ...
    def _regx_engine_rs(
        ...
        formula = f"{formula[0]} + offset(log(w5_weights))" + (
            f" | {formula[1]}" if len(formula) > 1 else ""
        )
        dfr = dfr.copy()
        ...
        dfr[["w5xhh", "w2"]] = (
            dfr[["weekly_trips", "w1"]] if method == "old" else dfr[["w5xhh", "w2"]]
        )

        w5x_fill = self._nan_fill(dfr, col_grby, "w5xhh", "weekly_trips")
        dfr["w5_weights"] = (
            dfr["w5xhh"].div(dfr["weekly_trips"]).fillna(w5x_fill)
        )  # offset
        reg = r2.r["run_model"](form, formula, pd2r.py2rpy(dfr))
        col_year = (
            ["purpose", "surveyyear"] if "surveyyear" in col_grby else ["purpose"]
    ...
    def _regx_engine_py(
        self, form: str, formula: str, dfr: pd.DataFrame, tfn_ttype: List
    ):
        # model setup
        ...
        dfr = (
            dfr.groupby(
                tfn_ttype + (["surveyyear"] if "surveyyear" in col_grby else []),
                observed=False,
            )[["weekly_trips"]]
            .sum()
            .reset_index()
    ...
    def _test_regression(self, dfr: pd.DataFrame, form: str, formula: str):
        ...
        str_form = f"{str_form[0]} + w2 + offset(log(w5_weights))" + (
            f" | {str_form[1]}" if len(str_form) > 1 else ""
        )
        ...
        a["w5_weights"] = (
            a[col_weight].div(a["weekly_trips"]).fillna(w5x_fill)
        )  # offset
        ...
        a = dfr.copy()
        str_form = formula.split("|")
        str_form = f"{str_form[0]} + offset(log(w5_weights))" + (
            f" | {str_form[1]}" if len(str_form) > 1 else ""
        )
        a[col_grby] = a[col_grby].astype(str).astype("category")
    ...
    def _dfr_to_dcat(dfr: pd.DataFrame, col_used: Union[str, List]) -> pd.DataFrame:
        ...
        for col in dfr.columns:
            ...
            tmp.rename(
                columns={itm: f"{col}_{itm}" for itm in tmp.columns}, inplace=True
            )
    ...
    @staticmethod
    def _nan_fill(
        dfr: pd.DataFrame, col_grby: List, col_enum: str, col_deno: str
    ) -> pd.Series:
        out = dfr[col_enum]
        for lev, key in enumerate(col_grby):
            ...
            out = (
                dfr_enum.div(dfr_deno)
                if lev == 0
                else out.fillna(dfr_enum.div(dfr_deno))
            )
