...
hb_csv <- read_csv("C:/Users/Pluto/Documents/Trip_rate_testing/classified_nts_trip_rates_v3.2.csv")
...
hb_csv <- hb_csv %>% 
  select(age_work_status, gender, hh_adults, cars, soc_cat, ns_sec, tfn_area_type, trip_purpose, weekly_trips, trip_weights, W2) %>% 
...
model_forms <- model_forms %>% 
  mutate(formula = case_when(
    aws == 1 & mod_form == "nb" ~ str_c(child_predictors, " + offset(log(trip_rate))"),
    aws == 1 & mod_form != "nb" ~ str_c(child_predictors, " | 1"),
    aws != 1 & p %in% 1:2 & mod_form == "nb" ~ str_c(com_bus_predictors, " + offset(log(trip_rate))"),
    aws != 1 & p %in% 1:2 & mod_form != "nb" ~ str_c(com_bus_predictors, " | 1"),
    aws != 1 & p %in% 3:8 & mod_form == "nb" ~ str_c(oth_predictors, " + offset(log(trip_rate))"),
    aws != 1 & p %in% 3:8 & mod_form != "nb" ~ str_c(oth_predictors, " | 1"))) %>% 
  mutate(response = ifelse(mod_form == "nb", "weekly_trips", "weekly_trips"),
...
for(i in 1:length(model_forms)){
  ...
  if(cur_iter$mod_form == "nb"){
    ...
    mod <- glm.nb(formula = as.formula(cur_iter$formula),
                  data = cur_df)
...
write_csv(trip_rates_out, "C:/Users/Pluto/Documents/Trip_rate_testing/hb_trip_rates_v3.2_offset.csv")
